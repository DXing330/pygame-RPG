...
def check_element_spell(s_pc, m_npc):
	if s_pc.element == "Fire":
		if m_npc.element == "Fire" or m_npc.element == "Water":
			...
			new_atk = s_pc.power
		#fire spells have a random chance to burn
		if "Fire" not in m_npc.element:
			if m_npc.health > 0:
				num = random.randint(0, m_npc.health)
				if num <= s_pc.power:
					if m_npc.status == None:
						m_npc.status = "Burn"
					elif "Burn" not in m_npc.status:
						m_npc.status += " Burn"
		...
		if m_npc.element == "Air" or m_npc.element == "Fire":
			...
			new_atk = s_pc.power
		#air spells have a chance to cut and bleed
		if "Air" not in m_npc.element:
				if m_npc.health > 0:
					num = random.randint(0, m_npc.health)
					if num <= new_atk:
						if m_npc.status == None:
							m_npc.status = "Bleed"
					elif "Bleed" not in m_npc.status:
							m_npc.status += " Bleed"
		...
		if num <= new_atk:
			if m_npc.status == None:
				m_npc.status = "Prone"
		elif "Prone" not in m_npc.status:
				m_npc.status += " Prone"
		...
		if m_npc.element == "Water" or m_npc.element == "Earth":
			...
			new_atk = s_pc.power
		if "Water" not in m_npc.element:
				if m_npc.health > 0:
					num = random.randint(0, m_npc.health)
					if num <= new_atk:
						if m_npc.status == None:
							m_npc.status = "Slog"
					elif "Slog" not in m_npc.status:
							m_npc.status += " Slog"
		...
		if m_npc.element == "Dark":
			...
			new_atk = s_pc.power
		if "Dark" not in m_npc.element:
				if m_npc.health > 0:
					num = random.randint(0, m_npc.health)
					if num <= new_atk:
						if m_npc.status == None:
							m_npc.status = "Decay"
					elif "Decay" not in m_npc.status:
							m_npc.status += " Decay"
	return new_atk
