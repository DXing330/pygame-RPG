...
def pet_pick_random_healthy_hero(heroes_party):
	...
	try:
		if len(heroes_party) > 1:                                        
			...
			hero = heroes_party[x]
			#don't pick golems
			if hero.health <=0 or "Totem" in hero.name:
...
def pet_pick_random_injured_hero(heroes_party):
	...
	try:
		if len(heroes_party) > 1:                                        
			...
			if hero.health <=0 or "Totem" in hero.name or hero.health >= hero.maxhealth:
				hero = pet_pick_random_healthy_hero(heroes_party)
			return hero
		else:
			hero = heroes_party[0]
			if "Totem" not in hero.name and hero.health > 0:
				return hero
			else:
				hero = Player_PC("nothing", 1, 0, 0, 0, 0, 0, 0, 0)
				return hero
...
def pick_lowest_health(h_p):
	...
	for z in range(0, len(h_p)):
		...
		for hero in h_p:
			...
			if hero.health > 0 and "Totem" not in hero.name:
